...
LABEL_CANDIDATES = ("class","genre","subgenre","sub_genre","style","main_genre")
...
def parse_args():
    ...
    p.add_argument("--label", default="class",
...
def main():
    ...
    if "songs" in os.path.basename(csv_path).lower():
        raise SystemExit(f"Refusing to train on lookup file: {csv_path}")

    # Load + normalize headers
    df = pd.read_csv(csv_path)
    print(f"[INFO] Raw columns: {len(df.columns)}")
    print(f"[INFO] First 10 cols: {list(df.columns)[:10]}")
    print(f"[INFO] Last 10 cols:  {list(df.columns)[-10:]}")
    df.columns = [str(c).strip().lower() for c in df.columns]

    # Pick/validate label
    req = (a.label or "").strip().lower()
    label = req if req in df.columns else next((c for c in LABEL_CANDIDATES if c in df.columns), None)
    if not label:
        raise SystemExit(f"Could not find a label column. Tried '{a.label}' and {LABEL_CANDIDATES}. "
                         f"Tail cols: {list(df.columns)[-25:]}")
    ...
    clf = Pipeline([
        ...
        ("rf", RandomForestClassifier(n_estimators=a.n_estimators, n_jobs=-1, random_state=a.random_state))
    ])
    clf.fit(Xtr, ytr)
